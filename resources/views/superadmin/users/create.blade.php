...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm relative overflow-hidden">
        ...
        <div class="border-b border-slate-100 pb-5 mb-6 relative">
            <h2 class="text-2xl font-extrabold text-slate-800 font-display">Daftarkan Akun Siswa</h2>
            <p class="text-slate-500 mt-1 text-sm">Pastikan NIS siswa sudah terdaftar di Daftar NIS Terverifikasi terlebih dahulu.</p>
        </div>
        ...
        <form action="{{ route('superadmin.users.store') }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" id="name" name="name" required placeholder="Nama Siswa" value="{{ old('name') }}"
                ...
                <div>
                    <label for="nis" class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">NIS (Wajib Terverifikasi)</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" id="username" name="username" required placeholder="Contoh: ahmad2024" value="{{ old('username') }}"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all">
                </div>

                <div>
                    <label for="email" class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Email (Opsional)</label>
                    <input type="email" id="email" name="email" placeholder="Contoh: ahmad@siswa.sch.id" value="{{ old('email') }}"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" id="kelas" name="kelas" required placeholder="Contoh: XI TKR 1" value="{{ old('kelas') }}"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all">
                </div>
            </div>
