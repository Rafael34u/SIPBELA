...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm relative overflow-hidden">
        ...
        <div class="border-b border-slate-100 pb-5 mb-6 relative">
            <h2 class="text-2xl font-extrabold text-slate-800 font-display">Edit Akun Admin</h2>
            <p class="text-slate-500 mt-1 text-sm">Ubah detail profil untuk <strong>{{ $admin->name }}</strong>.</p>
        </div>
        ...
        <form action="{{ route('superadmin.admins.update', $admin) }}" method="POST" class="space-y-5 relative">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" id="name" name="name" required placeholder="Nama Admin" value="{{ old('name', $admin->name) }}"
                ...
                <div>
                    ...
                    <select id="role" name="role" required
                        ...
                        <option value="admin_bengkel" {{ old('role', $admin->role) == 'admin_bengkel' ? 'selected' : '' }}>Admin Bengkel</option>
                        <option value="admin_perpus" {{ old('role', $admin->role) == 'admin_perpus' ? 'selected' : '' }}>Admin Perpustakaan</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" id="username" name="username" required placeholder="Contoh: admin_perpus1" value="{{ old('username', $admin->username) }}"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all">
                </div>

                <div>
                    <label for="email" class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Email (Opsional)</label>
                    <input type="email" id="email" name="email" placeholder="Contoh: admin@sekolah.sch.id" value="{{ old('email', $admin->email) }}"
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all">
                </div>
            </div>
            ...
            <div class="bg-amber-50/50 border border-amber-100 p-4 rounded-2xl">
                <p class="text-xs text-amber-700 leading-normal">
                    💡 <strong>Opsional:</strong> Kosongkan kolom password di bawah ini jika tidak ingin mengubah password lama.
                </p>
            </div>
